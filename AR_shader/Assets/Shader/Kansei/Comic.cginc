                ...
                float2 texCoord : TEXCOORD) : SV_Target
{
    ...
    //srgb = lerp(float3(0, 0, 0) * y, srgb, mask);
    ...
    float g_edge = gray(sqrt(sx*sx + sy*sy));

    float4 c11_2 = rgba(uv.xy);   //そのままの色
    ...
    // エッジと閾値を複数設定して色を変える ８階調(0,36,72,109,145,182,218,255)をネットで引っ張ってきたが、見た目が悪かったので７つに変更値は255/7
    if (g_edge > 0.4) {
        ...
        if (g < 0.14) {
            ...
            return fixed4( 0.28,  0.28,  0.28, 1);
        } else if (g < 0.42) {
            return fixed4(0.42, 0.42, 0.42, 1);
        } else if (g < 0.56) {
            return fixed4(0.56, 0.56, 0.56, 1);
        } else if (g < 0.71) {
            return fixed4(0.71, 0.71, 0.71, 1);
        } else if (g < 0.85) {
            return fixed4(0.85, 0.85, 0.85, 1);
        } else {
